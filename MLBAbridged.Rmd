...
initializeSaves <- function(){
    write.csv("cleared", file = "C:/Users/jorda/OneDrive/Documents/MLB DFS 2023/ts.csv")
    write.csv("cleared", file = "C:/Users/jorda/OneDrive/Documents/MLB DFS 2023/ps.csv")
...
getps <- function(){
  tryCatch(
    expr={
      psSaved <-read.csv("C:/Users/jorda/OneDrive/Documents/MLB DFS 2023/ps.csv")
...
getts <- function(){
  tryCatch(
    expr={
      tsSaved <-read.csv("C:/Users/jorda/OneDrive/Documents/MLB DFS 2023/ts.csv")
...
getBuiltLineups <- function(){
  tryCatch(
    expr={
      builtLineups <-as.data.frame(read.csv("C:/Users/jorda/Downloads/DKLineupsMLB.csv"))
      blList1 <- list()
      blList2 <- list()
      blList3 <- list()
      blList4 <- list()
...
final_export <- read.csv("C:/Users/jorda/Downloads/DKLineupsMLB.csv")
...
p_hrs <- max(p_to_sim$HRs[1]/abr, 0)
p_s <- max(p_to_sim$Singles[1]/abr, 0)
p_d <- max(p_to_sim$Doubles[1]/abr, 0)
p_w <- max(p_to_sim$Walks[1]/abr, 0)

p_abr <- c(p_hrs, p_s, p_d, p_w)

p_out <- max(1-sum(p_abr), 0)
...
playerTeam <- oprojections %>% select(Name, Team)
result <- simGames(oprojections, batter_lines, pitcher_lines, batting_orders)
result <- merge(result, playerTeam, all.x = T)
result$build_proj <- as.numeric(result$build_proj)
result <- result[order(result$build_proj, decreasing = T),]
View(result)

```


### Game sim lineup builder
```{r game-sim-builder}
...
matrix_vector <- c(vsalary, vsalary, vplayers, v1b, v2b, v3b, vss, vc, vOF, vP,  vlocks, vbans, vstacks, vteams) #vteams must be listed last
...
const.rhs = c(50000, min_salary,  10,   1,   1,    1,    1,   1,   3,     2, sum(vlocks), 0, 7)
const.dir = c("<=" ,       ">=","==","==","==", "==", "==","==","==",  "==",          "==", "==", ">=")
